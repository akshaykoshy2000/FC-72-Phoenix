...
class View extends Component {
  ...
  async componentWillMount() {
    if (this.props.patient !== null) {
      var patient = await this.state.patient.methods
        .patients(this.state.account)
        .call();
      var count = patient.fileCount;
      for (var i = 0; i < count; i++) {
        const file = await this.state.patient.methods
          .files(this.state.account, i)
          .call();
        this.setState({
      ...
      return (
        <div className="contact" key={files.fileHash}>
          <li className="collection-item avatar">
            <i className="material-icons circle blue darken-2">
              Insert_drive_file
            </i>
            <p>
              <b>{files.fileName}</b>
            ...
            <p>
              <i>Uploaded on: {files.datetime}</i>
            </p>
